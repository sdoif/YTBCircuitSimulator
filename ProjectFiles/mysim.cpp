...
int main()
{
  ...
  //Intialisation
  for(int l=input.size()-1; l>=0; l--){
    vector<string> line = input[l];
    //Resistor processing
    if(line[0].find('R')==0){
      cout<<"Detected Component: "<<line[0]<<endl;
    ...
    if(line[0].find('V')==0){
      ...
      //Check if connected to reference node
      if(stoi(line[2]) == 0){
        //Inserting 1 into respective node and 0 other entries in same row
        for(int e = 0; e<node_max ; e++){
          if(e==stoi(line[1])-1){
            con_s(stoi(line[1])-1, stoi(line[1])-1) = 1;
          }else{
            con_s(stoi(line[1])-1, e) = 0;
          }
        }
    ...
    for(int i1=1; i1<=(node_max); i1++){
      for(int j1=1; j1<=(node_max); j1++)
      cout<<"G"<<i1<<1<<":"<<con_s(i1-1,j1-1)<<" ";
    }
    cout<<"\n";
  }
